...
import { products, categories, findProduct } from "@/lib/catalog";
...
function Seller() {
  ...
  const sales = orders.flatMap((o) => o.items).reduce((s, i) => {
    ...
    <div className="mx-auto max-w-[1400px] px-3 py-6 sm:px-4">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Hola, {user.name}. Aquí puedes gestionar tu catálogo.</p>
        </div>
        <Button className="rounded-full bg-primary text-primary-foreground"><Plus className="mr-1 size-4" /> Publicar producto</Button>
      </div>
