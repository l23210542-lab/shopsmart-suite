  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-xl px-4 py-20 text-center">
        ...
        <p className="mt-2 text-muted-foreground">Explora el catálogo y añade tus primeros productos.</p>
        <Link to="/products" search={{} as any}>
          <Button className="mt-6 rounded-full bg-primary text-primary-foreground">Ver productos</Button>
        </Link>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-[1400px] px-3 py-6 sm:px-4">
      <div className="grid gap-6 md:grid-cols-[1fr_320px]">
            ...
            {items.map((item) => {
              ...
              return (
                <div key={item.productId} className="flex gap-4 py-4">
                  <Link to="/product/$id" params={{ id: p.id }} className="size-28 shrink-0 overflow-hidden rounded-md bg-muted">
                    <img src={p.image} alt={p.name} className="size-full object-cover" />
                  </Link>
                  <div className="flex flex-1 flex-col">
                    <Link to="/product/$id" params={{ id: p.id }} className="font-semibold hover:text-price">{p.name}</Link>
                    ...
                    <div className="mt-2 flex items-center gap-3">
                      <div className="flex items-center rounded-full border">
                        <button onClick={() => update(p.id, item.quantity - 1)} className="px-2 py-1 hover:bg-accent" aria-label="Disminuir"><Minus className="size-3" /></button>
                        <span className="min-w-8 text-center text-sm font-semibold">{item.quantity}</span>
                        <button onClick={() => update(p.id, Math.min(p.stock, item.quantity + 1))} className="px-2 py-1 hover:bg-accent" aria-label="Aumentar"><Plus className="size-3" /></button>
                      </div>
                      <button onClick={() => remove(p.id)} className="flex items-center gap-1 text-xs text-destructive hover:underline">
        ...
        <aside className="h-fit rounded-xl border bg-card p-5 shadow-card md:sticky md:top-32">
          ...
          <Link to="/products" search={{} as any}>
            <Button variant="ghost" className="mt-2 w-full">Seguir comprando</Button>
          </Link>
        </aside>
      </div>
    </div>
