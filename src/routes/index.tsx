...
import { categories, products, productsByCategory } from "@/lib/catalog";
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-3 sm:px-4">
      ...
      <section className="relative mt-3 overflow-hidden rounded-2xl bg-hero-gradient px-6 py-14 text-nav-foreground sm:px-12 sm:py-20">
        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: "radial-gradient(circle at 20% 20%, white 1px, transparent 1px), radial-gradient(circle at 80% 60%, white 1px, transparent 1px)", backgroundSize: "40px 40px, 60px 60px" }} />
        <div className="relative max-w-2xl">
          ...
          <p className="mt-4 max-w-xl text-base text-nav-foreground/80 sm:text-lg">
            Marketplace ligero estilo Amazon construido con PHP 8, MariaDB y un toque de Raspberry. Miles de productos, vendedores locales, envío rápido.
          </p>
          ...
          return (
            ...
            >
              ...
              <div className="aspect-[4/3] overflow-hidden rounded-lg bg-muted">
                {sample && (
                  <img src={sample.image} alt={c.name} className="size-full object-cover transition group-hover:scale-105" />
                )}
              </div>
      ...
      <section className="mt-6 grid gap-3 rounded-xl border bg-card p-4 shadow-card sm:grid-cols-3">
        <Trust icon={<Truck className="size-5" />} title="Envío local rápido" desc="Despachos desde tu Raspberry Pi" />
        <Trust icon={<ShieldCheck className="size-5" />} title="Compra protegida" desc="Devoluciones gratis 30 días" />
        <Trust icon={<Cpu className="size-5" />} title="Hecho con Pi" desc="PHP + MariaDB + Tailwind" />
      </section>
