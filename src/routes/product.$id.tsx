...
export const Route = createFileRoute("/product/$id")({
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-xl p-12 text-center">
      ...
      <Link to="/products" search={{} as any} className="mt-4 inline-block text-price underline">Volver al catálogo</Link>
    </div>
  ),
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-3 py-6 sm:px-4">
      <nav className="mb-3 text-xs text-muted-foreground">
        ...
        <Link to="/products" search={{ cat: product.category, q: undefined } as any} className="hover:underline">{product.category}</Link> ·{" "}
      ...
      <div className="grid gap-6 rounded-xl border bg-card p-4 shadow-card md:grid-cols-[1fr_1.2fr_320px] md:p-6">
        ...
        <div>
          ...
          <Link to="/products" search={{} as any} className="mt-1 inline-block text-sm text-price hover:underline">
            por {product.seller}
          </Link>
          <div className="mt-2 flex items-center gap-2">
            <div className="flex">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className={`size-4 ${i < Math.round(product.rating) ? "fill-rating text-rating" : "text-muted-foreground"}`} />
              ))}
            </div>
          ...
          <ul className="mt-3 space-y-1 text-sm">
            <li className="flex items-center gap-2"><Check className="size-4 text-success" /> Stock disponible: {product.stock} unidades</li>
            <li className="flex items-center gap-2"><Check className="size-4 text-success" /> Vendido y enviado por {product.seller}</li>
          </ul>
        </div>
        ...
        <aside className="h-fit rounded-lg border bg-background p-4">
          ...
          <div className="mt-3 space-y-2">
            ...
            <Link to="/cart">
              <Button variant="secondary" className="w-full rounded-full">Ir al carrito</Button>
            </Link>
          </div>

          <div className="mt-4 space-y-1.5 text-xs text-muted-foreground">
            <p className="flex items-center gap-2"><ShieldCheck className="size-3.5" /> Compra protegida</p>
            <p className="flex items-center gap-2"><RotateCcw className="size-3.5" /> Devolución gratuita 30 días</p>
          </div>
        </aside>
      </div>
